...
treeInsert k d (Leaf key val)
  | k == key = Leaf k d
  | k < key = Branch ( Empty ) k d ( Leaf key val )
  | otherwise = Branch ( Leaf key val ) k d ( Empty )

treeInsert k d ( Branch ( left ) key val ( right ) )
  | k == key = Branch ( left ) k d ( right )
  | k < key = Branch ( treeInsert k d left) key val ( right )
  | otherwise = Branch ( left ) key val ( treeInsert k d right )
