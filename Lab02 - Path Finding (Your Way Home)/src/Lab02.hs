...
pathDistance []  = 0
pathDistance [s] = distance s
pathDistance (s:ss) = distance s + pathDistance ss
...
pathTime [] = 0
pathTime [s] = time s
pathTime (s:ss) = time s + pathTime ss
{-pathTime = \ _ -> -1 -}
...
getMeasured _ [] = []
getMeasured rqdDist (s:ss)
  | distance s < rqdDist  = [s] ++ getMeasured (rqdDist - distance s) ss
  | distance s >= rqdDist = [s]
  | otherwise             = []
...
findFastest rqdDist p
    | rqdDist > pathDistance a = Nothing
    | otherwise = Just (getHighestSpeed b)
  where a = getMeasured rqdDist p
        b = getPaths rqdDist p

getPaths :: Distance
         -> SegPath
         -> [ (SegPath, Speed) ]
getPaths _ [] = []
getPaths rqdDist (s:ss)
  | pathDistance (s:ss) < rqdDist = []
  | length (s:ss) > 0 = [pathAndSpeed] ++ getPaths rqdDist ss
  | otherwise = []
  where pathAndSpeed = ( getMeasured rqdDist (s:ss), pathSpeed ( getMeasured rqdDist (s:ss) ) )

getHighestSpeed :: [ (SegPath, Speed) ]
                -> (SegPath, Speed)
getHighestSpeed (s:ss) = getMaxTail s ss
...
getMaxTail (m, n) (s:ss) =
  if n < (snd s)
    then getMaxTail s ss
      else getMaxTail (m, n) ss
